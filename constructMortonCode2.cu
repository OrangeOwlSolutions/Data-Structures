...
__device__ const unsigned int Morton3D_encode_x_256[256] =
{
	0x00000000,
...
__device__ const unsigned int Morton3D_encode_y_256[256] = {
	0x00000000,
...
__device__ const unsigned int Morton3D_encode_z_256[256] = {
	0x00000000,
...
//__device__ __forceinline__ morton m3D_e_sLUT(const coord x, const coord y, const coord z) {
//	morton answer = 0;
//	for (unsigned int i = sizeof(coord); i > 0; --i) {
//		unsigned int shift = (i - 1) * 8;
//		answer = Morton3D_encode_x_256[(x >> shift) & EIGHTBITMASK];
//		//answer =
//		//	answer << 24 |
...
__global__ void mortonEncoder(const float3 * __restrict__ d_points, unsigned int * __restrict__ d_mortonCode, const bbox boundingBox, const int N) {

	const int tid = threadIdx.x + blockIdx.x * blockDim.x;
	
	if (tid > N) return;
	
	float x = (d_points[tid].x - boundingBox.lowerLeft.x) / (boundingBox.upperRight.x - boundingBox.lowerLeft.x);
	float y = (d_points[tid].y - boundingBox.lowerLeft.y) / (boundingBox.upperRight.y - boundingBox.lowerLeft.y);
	float z = (d_points[tid].z - boundingBox.lowerLeft.z) / (boundingBox.upperRight.z - boundingBox.lowerLeft.z);

	d_mortonCode[tid] = morton3D(x, y, z);
	//d_mortonCode[tid] = m3D_e_sLUT<unsigned int, float>(x, y, z);
...
int main(void)
{
	...
	unsigned int *h_mortonCode = (unsigned int *)malloc(N * sizeof(unsigned int));
	unsigned int *d_mortonCode; gpuErrchk(cudaMalloc(&d_mortonCode, N * sizeof(unsigned int)));
	...
	//gpuErrchk(cudaMemcpy(h_mortonCode, d_mortonCode, N * sizeof(unsigned int), cudaMemcpyDeviceToHost));

	//for (int k = 0; k < N; k++)
	//	std::cout << k << " " << h_mortonCode[k] << " " << h_points[k].x << " " << h_points[k].y << " " << h_points[k].z << "\n";
